...
def test_package_raise_missing_values():
    with pytest.raises(ValueError):
        Package(Path("/home"), None, "pkg", "url")  # type: ignore
    with pytest.raises(ValueError):
        Package(Path("/home"), "repo", None, "url")  # type: ignore
    with pytest.raises(ValueError):
        Package(Path("/home"), "repo", "pkg", None)  # type: ignore
...
def test_package_has_url_changed(git_url, package_url, expected_result):
    ...
    OutProcess = namedtuple("OutProcess", ["stdout"])

    def _run_bash(command, **kwargs):
        if "config" in command:
            return OutProcess(git_url)
        elif "rev-parse" in command:
            return OutProcess("false") if is_dir_local else OutProcess("true")
