...
from pearllib.exceptions import (
    PackageNotInRepoError,
    PackageNotInstalledError,
    RepoDoesNotExistError,
)
from pearllib.models import Package
from pearllib.pearlenv import PackageBuilder, PackageLoader, PearlEnvironment
...
@pytest.mark.parametrize(
    "home, expected_home",
    [
        pytest.param(None, "home-dir/.local/share/pearl"),
        pytest.param("home-dir/.local/share/pearl", "home-dir/.local/share/pearl"),
        pytest.param(
...
def test_config_filename():
    assert PearlEnvironment._get_config_filename(Path("/myhome2/pearl.conf"), False) == Path(
        "/myhome2/pearl.conf"
    )
    assert PearlEnvironment._get_config_filename(None, False) == Path(
        f"{os.environ['HOME']}/.config/pearl/pearl.conf"
    )
...
def test_load_repos_init_repo(tmp_path):
    ...
    with mock.patch(_MODULE_UNDER_TEST + ".subprocess") as subprocess_mock:
        repo_path = (
            pearl_home / "repos/c7b64f5f2c6bc1f457921bbac2886482b9540e8c4f56af799ec4946f34ebbc16"
        )
        assert PackageLoader._load_repos(
            pearl_home, ("https://github.com/pearl-hub/repo.git",)
        ) == [repo_path / "pearl-config/repo.conf"]

        assert subprocess_mock.run.call_args[0][0] == [
            "git",
            "clone",
            "--depth",
            "1",
...
@pytest.mark.parametrize("update_repos", [pytest.param(True), pytest.param(False)])
def test_load_repos_update_repo(update_repos, tmp_path):
    pearl_home = tmp_path / "home"
    pearl_home.mkdir()

    repo_dir = (
        pearl_home / "repos/c7b64f5f2c6bc1f457921bbac2886482b9540e8c4f56af799ec4946f34ebbc16/.git"
    )
    ...
    with mock.patch(_MODULE_UNDER_TEST + ".subprocess") as subprocess_mock:
        ...
        assert PackageLoader._load_repos(
            pearl_home, ("https://github.com/pearl-hub/repo.git",), update_repos=update_repos
        ) == [repo_path / "pearl-config/repo.conf"]
        if update_repos:
            assert subprocess_mock.run.call_args[0][0] == [
                "git",
...
def test_required_by(tmp_path):
    ...
    pearl_env = PearlEnvironment(home_dir, env_initialized=False)
    pearl_env._packages = packages
    requires = pearl_env.required_by(Package(home_dir, "repo-test", "pkg-test2", "/blah"))
