...
from pearllib.pearl import PearlCommand, _extract_packages, pearl
from pearllib.pearlenv import PearlEnvironment

_MODULE_UNDER_TEST = "pearllib.pearl"


def expected_pack_calls(
    install_pkg=0, update_pkg=0, emerge_pkg=0, info_pkg=0, remove_pkg=0, list_pkg=0, create_pkg=0
):
    return {
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ["install", "pkg1"], expected_pack_calls(install_pkg=1), expected_syst_calls()
        ),
        pytest.param(
            ["install", "pkg1", "pkg2"], expected_pack_calls(install_pkg=1), expected_syst_calls()
        ),
        pytest.param(
        ...
        pytest.param(
            ["info", "pkg1", "pkg2", "pkg3"], expected_pack_calls(info_pkg=1), expected_syst_calls()
        ),
        pytest.param(
        ...
        pytest.param(["list"], expected_pack_calls(list_pkg=1), expected_syst_calls()),
        pytest.param(["search", "pattern"], expected_pack_calls(list_pkg=1), expected_syst_calls()),
        pytest.param(
            ["create", "name", "dest_dir"], expected_pack_calls(create_pkg=1), expected_syst_calls()
        ),
        pytest.param(["init"], expected_pack_calls(), expected_syst_calls(init=1)),
        pytest.param(["update"], expected_pack_calls(), expected_syst_calls(update=1)),
        pytest.param(["remove"], expected_pack_calls(), expected_syst_calls(remove=1)),
    ],
)
def test_pearl(args, expected_pack_call_counts, expected_syst_call_counts, tmp_path):
    ...
    with mock.patch(_MODULE_UNDER_TEST + ".pack") as pack_mock, mock.patch(
        _MODULE_UNDER_TEST + ".syst"
    ) as syst_mock, mock.patch(_MODULE_UNDER_TEST + ".verify_runtime_deps") as verify_mock:
...
def test_extract_packages_raises(tmp_path):
    ...
    pearl_env.lookup_package.side_effect = PackageNotInRepoError("")
    _extract_packages(
        pearl_env, PearlCommand.REMOVE, PackageArgs(packages=["pkg-test"], force=True)
    )
