...
@pytest.mark.parametrize(
    'args, args_packages, expected_pack_call_counts, expected_syst_call_counts',
    [
        pytest.param(
            ['install'],
            ['pkg'],
...
def test_pearl(args, args_packages, expected_pack_call_counts, expected_syst_call_counts, tmp_path):
    ...
    with mock.patch(_MODULE_UNDER_TEST + '.pack') as pack_mock, \
        ...
        pack_mock.closure_dependency_tree.return_value = args_packages

        pearl(['-c', str(home_dir / 'pearl.conf')] + args + args_packages, pearl_home_dir=pearl_home_dir)
...
def test_pearl_error(command, func_name, tmp_path):
    ...
    with mock.patch(_MODULE_UNDER_TEST + '.pack') as pack_mock, \
        ...
        with pytest.raises(SystemExit) as exc:
            pearl(['-c', str(home_dir / 'pearl.conf'), command] + args_packages, pearl_home_dir=pearl_home_dir)
