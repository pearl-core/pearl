...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(0, "5.0.11(1)-release", True),
        pytest.param(0, "4.2", True),
        pytest.param(0, "4.1", True),
        pytest.param(0, "5", True),
        pytest.param(0, "3.9", False),
        pytest.param(0, None, False),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(0, "git version 1.8.6", True),
        pytest.param(0, "git version 1.8.5", True),
        pytest.param(0, "git version 2", True),
        pytest.param(0, "git version 1.7.9", False),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param("echo ciao\nfalse", None, "ciao\n", 1),
        pytest.param("echo ciao", None, "ciao\n", 0),
        pytest.param("echo ciao\nasfpoji", None, "ciao\n", 127),
        pytest.param("read var; echo $var\n", "ciao\n", "ciao\n", 0),
        # No confirm
        pytest.param(
            "read var; [[ -z $var ]] && echo ciao; read var2; echo $var2", "", "ciao\n\n", 0
        ),
    ],
)
...
def test_run_pearl_bash(tmp_path):
    ...
    info "Test"
    """
    result = run_pearl_bash(
        script, pearl_env, capture_stdout=True, capture_stderr=True, enable_xtrace=False
    )
...
def test_run_pearl_bash_enable_xtrace(tmp_path):
    ...
    script = "echo hello"
    result = run_pearl_bash(
        script, pearl_env, capture_stdout=True, capture_stderr=True, enable_xtrace=True
    )
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(["Y"], False, True),
        pytest.param(["N"], True, False),
        pytest.param(["y"], False, True),
        pytest.param(["n"], True, False),
        pytest.param(["R", "c", "q", "Y"], False, True),
        pytest.param(["R", "c", "q", "N"], False, False),
        pytest.param(["R", ""], False, False),
        pytest.param(["R", ""], True, True),
    ],
)
